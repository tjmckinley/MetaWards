...
// Distances are not included in the play matrix.
{
    ...
    if ((locf) == NULL)
    {
        ...
        printf("Location files %s found, great, carry on.\n", par->PositionName);
...
network *BuildWardsNetworkDistanceIdentifiers(parameters *par)
{
    ...
    if ((inF) == NULL)
    {
        ...
        printf("Identifier file %s found, great, carry on.\n", par->IdentifierName);
    ...
    if ((inF) == NULL)
    {
        ...
        printf("Identifier 2 file %s found, great, carry on.\n", par->IdentifierName2);
    i = 1;
    while (!feof(inF))
    {
        fscanf(inF, "%s %d\n", temp, &id);
...
int ApplyStaticDistanceCutoff(network *net, parameters *par)
{
    ...
    { // loop through nodes
        ...
        { //loop through links of node i
            checksum += play[j].weight;
            if (play[j].distance > cutoff)
            { //	collect proportion of players to redistribute
        ...
        for (j = wards[i].begin_to; j < wards[i].end_to; j++)
        {
            ...
            { //	collect proportion of workers to redistribute
                // and set weight of those links to 0
...
void BuildPlayMatrix(network *net, parameters *par)
{
    ...
    { //rescale appropriately!
        ...
        if (!strcmp(par->PlayName, par->WorkName))
        { // if the input file is the same as the PlayMatrix, normalise!
    ...
    while (!feof(inFile2))
    { // Read in the real number of play susceptibles.
...
void RescalePlayMatrix(network *net, parameters *par)
{
    ...
    { //if we are making people stay at home, then do this loop through nodes
        ...
        { //rescale appropriately!
            ...
            if (llist[j].ifrom != llist[j].ito)
            {                                                                     // if it's not the home ward, then reduce the number of play movers
...
void RecalculatePlayDenominatorDay(network *net, parameters *par)
{
    ...
    for (i = 1; i <= net->nnodes; i++)
        wards[i].Denominator_PD = wards[i].Denominator_P = 0.0;
...
void RecalculateWorkDenominatorDay(network *net, parameters *par)
{
    ...
    int sum = 0;
    for (i = 1; i <= net->nnodes; i++)
        wards[i].Denominator_D = wards[i].Denominator_N = 0.0;
...
int **InitialiseInfections(network *net)
{
    ...
    for (i = 0; i < N_INF_CLASSES; i++)
        inf[i] = (int *)calloc(sizeof(int), net->nlinks + 1);
...
int **InitialisePlayInfections(network *net)
{
    ...
    for (i = 0; i < N_INF_CLASSES; i++)
        inf[i] = (int *)calloc(sizeof(int), net->nnodes + 1);
                               ...
                               gsl_rng *r, char C, int **inf)
{
    ...
    else if (C == 'N')
    {
        ...
        while (links[j].ito != i && links[j].ifrom != i)
            j++;
...
void SeedInfectionAtNode(network *net, parameters *par, int node_seed, int **inf, int **pinf)
{
    ...
    while (links[j].ito != node_seed || links[j].ifrom != node_seed)
        j++;
              ...
              int s)
{
    ...
    if (s < 0)
    {
        ...
        SeedInfectionAtNode(net, par, to_seed[s], inf, playinf);
#endif
    ...
    while (infecteds != 0 || i < 5)
    {
        ...
        if (infecteds > par->GlobalDetectionThresh)
            trigger = 1;
        if (trigger == 1)
        {
...
void GetMinMaxDistances(network *net, double *min, double *max)
{
    ...
    *max = -1;
    ...
    for (i = 1; i <= net->nlinks; i++)
    {
        ...
        if (links[i].distance > *max)
            *max = links[i].distance;
        if (links[i].distance < *min)
            *min = links[i].distance;
    }
...
int ExtractData(network *net, int **inf, int **pinf, int t, FILE **files)
{
    ...
    for (j = 1; j <= net->nnodes; j++)
        TotalNewInfWard[j] = TotalInfWard[j] = 0;
...
#ifdef SELFISOLATE
                if (i > 4 && i < 10)
                    IsDangerous[links[j].ito] += inf[i][j];
#endif
...
#ifdef SELFISOLATE
                if (i > 4 && i < 10)
                    IsDangerous[i] += pinf[i][j];
#endif
            }
            if (i < (N_INF_CLASSES - 1) && TotalInfWard[j] > 0)
                nInfWards[i]++;
    ...
    for (i = 0; i < N_INF_CLASSES; i++)
    {
        ...
        { //commuters
            if (inf[i][j] != 0)
            {
                ...
                { // sum up all the infectious classes.
        ...
        { //non-commuters
            ...
            if (i == 2)
                fprintf(outF, "%d ", TotalInfections[j]); // prevalence up to i=2
...
int ExtractDataForGraphicsToFileIncidence(network *net, int **inf, int **pinf, FILE *outF)
{
    ...
    { //loop over classes
        ...
        if (i == 2)
            fprintf(outF, "%d ", TotalInfWard[i][j]); //incidence
...
void Iterate(network *net, int **inf, int **playinf, parameters *par, gsl_rng *r, int t)
{
...
#endif
    for (i = 0; i < N_INF_CLASSES; i++)
    { //setting up the day and nighttime FOI

        if (par->beta[i] * par->ContribFOI[i] > 0)
        {

            for (j = 1; j <= net->nlinks; j++)
            { // Deterministic movements (to work).
            ...
            for (j = 1; j <= net->nnodes; j++)
            { // playmatrix loop FOI loop (random movements)
                ...
                if (playinf[i][j] != 0)
                {
                    ...
                    while (moving > 0 && k <= wards[j].end_p)
    ...
    { // actual new infections for fixed movements
        ...
        if (links[j].distance < cutoff)
        { // if distance is below cutoff (reasonable distance) infect in work ward

            if (wards[links[j].ito].DayFOI > 0)
        ...
        else if (wards[links[j].ifrom].DayFOI > 0)
        { // if distance is too large infect in home ward with day FOI
        ...
        if (wards[links[j].ifrom].NightFOI > 0)
        { //nighttime infection of workers j
    ...
    { // playmatrix loop.
        ...
        if ((staying + moving) > 0)
        { // infect people staying at home damnit!!!!!!!!
...
void IterateWeekend(network *net, int **inf, int **playinf, parameters *par, gsl_rng *r, int t)
{
    ...
    { // set all relevant variables to 0;
        wards[i].DayFOI = wards[i].NightFOI = 0.0;
        for (j = 0; j < N_INF_CLASSES; j++)
            weinf[j][i] = 0;
        wesuscept[i] = newly_infected[i] = 0;
    }

    for (i = 0; i < N_INF_CLASSES; i++)
    { //setting up the day and nighttime FOI and susceptibles

        if (par->beta[i] * par->ContribFOI[i] > 0)
        {

            for (j = 1; j <= net->nlinks; j++)
            { // Sum up work movements of infection.
                if (i == 0)
                    wesuscept[links[j].ifrom] += (int)links[j].suscept; // sum up work susceptibles once
                if (inf[i][j] > 0)
                    weinf[i][links[j].ifrom] += inf[i][j]; // sum up infecteds in the work matrix (pool together)
            }

            for (j = 1; j <= net->nnodes; j++)
            { // FOI and susceptibles loop
                if (i == 0)
                    wesuscept[j] += (int)wards[j].play_suscept; // sum up susceptibles once
                if (playinf[i][j] > 0)
                    weinf[i][j] += playinf[i][j]; // add to work the infecteds in the play matrix (pool together)

                if (weinf[i][j] > 0)
                { // distribute infecteds and update FOI
                    ...
                    while (moving > 0 && k < wards[j].end_we)
                    { // distributing people across play wards
    ...
    for (i = N_INF_CLASSES - 2; i >= 0; i--)
    { // recovery, move through classes backwards. this can stay the same for weekends
    ...
    for (j = 1; j <= net->nnodes; j++)
    { // weekend loop where we do the "pooled" infecting
    ...
    for (j = 1; j <= net->nnodes; j++)
    {
        ...
        if (newly_infected[j] > 0)
        {
            ...
            for (k = wards[j].begin_to; k < wards[j].end_to; k++)
            { // now for the work matrix
                //printf("ifrom %d \n",links[k].ifrom);
...
void CalculateWardRisk(network *net, double *RiskRA, int **inf, int **pinf, parameters *par)
{
    ...
    for (i = 1; i <= net->nnodes; i++)
    {
        if (tot[i] > 0.0)
        {
            for (j = wards[i].begin_to; j <= wards[i].end_to; j++)
            {
                if (RiskRA[links[j].ito] > -1)
                    RiskRA[links[j].ito] += plinks[j].weight * tot[i];
                if (RiskRA[i] > -1)
                    RiskRA[i] += plinks[j].weight * tot[i];
            }
        }
    }
    free(tot);
...
void FindWardsToVaccinate(network *net, size_t *wardsRA, int **inf, int **pinf, parameters *par)
{
    ...
    for (i = 1; i <= net->nnodes; i++)
    {
        ...
        if (wardsRA[i] == 0)
        { // if ward has not already been vaccinated or has not already been selected for vaccination
            Local = TotalWardInfectious(wards[i], i, inf, pinf, par);
            Global += Local;
            if (Local > par->LocalVaccinationThresh)
            { // is ward infectious
...
void FindNeighboursToVaccinate(network *net, int iward, size_t *wardsRA, parameters *par)
{
    ...
    for (i = wards[iward].begin_to; i <= wards[iward].end_to; i++)
    {
        if (i != wards[iward].self_w && wardsRA[links[i].ito] == 0)
        {
            ...
            if (temp > par->NeighbourWeightThreshold)
                wardsRA[links[i].ito] = 1;
        }
    }
...
int IsWardInfected(node ward, int iward, int **inf, int **pinf, parameters *par)
{
    ...
    for (i = 0; i < N_INF_CLASSES - 1; i++)
        total += pinf[i][iward];
    if (total > par->LocalVaccinationThresh)
        return 1;
...
void HaltMovementsInLink(network *net, int ilink, int **inf, int **pinf, double prop)
{
    ...
    for (i = 0; i < N_INF_CLASSES; i++)
    {                                                       // for each infectious class, siphon a proportion of people moving
