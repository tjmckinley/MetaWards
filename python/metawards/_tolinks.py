...
class ToLinks:
    """This is a container class for ToLinks. This uses Numpy arrays
    ...
    def __init__(self, N: int=0):
        ...
        if N <= 0:
            ...
            self.ifrom = np.full(N, -1, int_t)
            self.ito = np.full(N, -1, int_t)

            self.weight = np.full(N, 0.0, float_t)
            self.suscept = np.full(N, 0.0, float_t)
            self.distance = np.full(N, 0.0, float_t)

            self.A = np.full(N, -1, int_t)
